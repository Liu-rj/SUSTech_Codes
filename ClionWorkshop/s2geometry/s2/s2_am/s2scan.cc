...
IndexScanDesc
s2beginscan(Relation r, int nkeys, int norderbys)
{
    IndexScanDesc scan;
    GISTSTATE *giststate;
    GISTScanOpaque so;
...
void
s2rescan(IndexScanDesc scan, ScanKey key, int nkeys,
         ScanKey orderbys, int norderbys)
{
    ...
    {
        /* first time through */
        Assert(so->queueCxt == so->s2State->scanCxt);
        first_time = true;
    } else if (so->queueCxt == so->s2State->scanCxt)
    {
        /* second time through */
        so->queueCxt = AllocSetContextCreate(so->s2State->scanCxt,
                                             "GiST queue context",
    ...
    if (scan->xs_want_itup && !scan->xs_hitupdesc)
    {
        ...
        so->s2State->fetchTupdesc = CreateTemplateTupleDesc(natts);
        for (attno = 1; attno <= nkeyatts; attno++)
        {
            TupleDescInitEntry(so->s2State->fetchTupdesc, attno, NULL,
        ...
        {
            /* taking opcintype from s2State->tupdesc */
            TupleDescInitEntry(so->s2State->fetchTupdesc, attno, NULL,
                               TupleDescAttr(so->s2State->leafTupdesc,
                                             attno - 1)->atttypid,
                               -1, 0);
        }
        scan->xs_hitupdesc = so->s2State->fetchTupdesc;

        /* Also create a memory context that will hold the returned tuples */
        so->pageDataCxt = AllocSetContextCreate(so->s2State->scanCxt,
    ...
    if (key && scan->numberOfKeys > 0)
    {
        ...
        for (i = 0; i < scan->numberOfKeys; i++)
        {
            ...
            fmgr_info_copy(&(skey->sk_func),
                           &(so->s2State->consistentFn[skey->sk_attno - 1]),
                           so->s2State->scanCxt);
    ...
    if (orderbys && scan->numberOfOrderBys > 0)
    {
        ...
        for (i = 0; i < scan->numberOfOrderBys; i++)
        {
            ...
            FmgrInfo *finfo = &(so->s2State->distanceFn[skey->sk_attno - 1]);
             ...
             */
            fmgr_info_copy(&(skey->sk_func), finfo, so->s2State->scanCxt);
