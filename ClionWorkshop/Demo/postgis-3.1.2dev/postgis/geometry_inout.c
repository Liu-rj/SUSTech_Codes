...
GetFace(double p[3])
{
	float8 temp[] = {Abs(p[0]), Abs(p[1]), Abs(p[2])};
	int face = temp[0] > temp[1] ? temp[0] > temp[2] ? 0 : 2 : temp[1] > temp[2] ? 1 : 2;
	if (p[face] < 0)
		face += 3;
	return face;
}
...
inline void
ValidFaceXYZtoUV(int face, const double p[3], double *pu, double *pv)
{
	switch (face)
	{
	case 0:
...
inline int
XYZtoFaceUV(double p[3], double *pu, double *pv)
{
	int face = GetFace(p);
...
Datum
point_to_geometry(PG_FUNCTION_ARGS)
{
	Point *point;
	LWPOINT *lwpoint;
	GSERIALIZED *geom;
	...
	double lng_radians = (M_PI / 180) * point->x;
	double lat_radians = (M_PI / 180) * point->y;
	double phi = lat_radians;
	double theta = lng_radians;
	double cosphi = cos(phi);
	double S2Point[] = {cos(theta) * cosphi, sin(theta) * cosphi, sin(phi)};

	double u, v;
	int face = XYZtoFaceUV(S2Point, &u, &v);
	double s = UVtoST(u);
	double t = UVtoST(v);

//	lwpoint = lwpoint_make2d(SRID_UNKNOWN, point->x, point->y);
	lwpoint = lwpoint_make2d(SRID_UNKNOWN, s, t);
...
Datum
geometry_to_point(PG_FUNCTION_ARGS)
{
	Point *point;
	POINT4D pt;
	GSERIALIZED *geom;
...
Datum
geometry_to_path(PG_FUNCTION_ARGS)
{
	PATH *path;
	LWLINE *lwline;
	LWGEOM *lwgeom;
...
Datum
path_to_geometry(PG_FUNCTION_ARGS)
{
	PATH *path;
	LWLINE *lwline;
	POINTARRAY *pa;
...
Datum
geometry_to_polygon(PG_FUNCTION_ARGS)
{
	POLYGON *polygon;
	LWPOLY *lwpoly;
	LWGEOM *lwgeom;
...
Datum
polygon_to_geometry(PG_FUNCTION_ARGS)
{
	POLYGON *polygon;
	LWPOLY *lwpoly;
	POINTARRAY *pa;
