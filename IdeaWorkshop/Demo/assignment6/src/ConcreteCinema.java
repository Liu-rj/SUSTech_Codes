...
public class ConcreteCinema implements Cinema {
    ...
    private List<MovieHall> movieHalls = new ArrayList<>();
    private int id = 1;
    ...
    public void addMovieHall(int capacity) {
        MovieHall movieHall = new MovieHall(id ++, capacity);
    ...
    public List<String> getAllMovieHallsCapacity() {
        ...
        movieHalls.sort(Comparator.comparingInt(MovieHall::getId));
        for (MovieHall hall : movieHalls) {
            stringList.add(hall.getId() + "-" + hall.getCapacity());
        }
        return stringList;
    }
    ...
    public void addMovie(String name, int runtime, int hallNumber, double price, int type, Time startTime) {
        Movie movie;
        if (type == 0) {
            movie = new OrdinaryMovie(name, runtime, hallNumber, price, startTime);
        } else {
            movie = new ThreeDMovie(name, runtime, hallNumber, price, startTime);
        }
        for (Movie movie1 : movies) {
            if (movie1.getHallNumber() == hallNumber) {
                if ((movie1.getStartTime().sort() < startTime.sort() + runtime + 20 && movie1.getSortedStartTime() > startTime.sort())|| (movie1.getSortedStartTime() < startTime.sort() && movie1.getSortedStartTime() + movie1.getRuntime() + 20 > startTime.sort())) {
                    return;
                }
            }
        }
        ...
        for (MovieHall hall : movieHalls) {
            if (hall.getId() == hallNumber){
                movie.ticketsLeft = hall.getCapacity();
            }
        }
    }
    ...
    public List<Movie> getMoviesFromMovieHallOrderByStartTime(int hallNumber) {
        ...
        for (Movie movie : movies) {
            if (movie.getHallNumber() == hallNumber){
                movieList.add(movie);
            }
        }
        movieList.sort(Comparator.comparingInt(Movie::getSortedStartTime));
        return movieList;
    }
    ...
    public double reserveMovie(int movieId, int arg) {
        return getMovieById(movieId).purchase(arg);
    }
    ...
    public double getOneMovieIncome(int movieId) {
        return getMovieById(movieId).income;
    }
    ...
    public List<Movie> getAvailableMoviesByName(Time currentTime, String name) {
        List<Movie> movieList = new ArrayList<>();
        for (Movie movie : movies) {
            if (movie.getSortedStartTime() > currentTime.sort() && movie.ticketsLeft > 0 && movie.getName().equals(name)){
                movieList.add(movie);
            }
        }
        movieList.sort(Comparator.comparingInt(Movie::getSortedStartTime));
