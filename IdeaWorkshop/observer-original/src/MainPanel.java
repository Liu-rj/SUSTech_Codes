...
public class MainPanel extends JPanel implements KeyListener, Subject<Ball> {
    ...
    public void notifyObservers(char keyChar) {
        paintingBallList.forEach(ball -> ball.update(keyChar));
    }

    @Override
    public void notifyObservers() {
        paintingBallList.forEach(ball -> ball.update(whiteBall));
    }
    ...
    public void restartGame() {
        ...
        while (paintingBallList.size() > 0) {
            removeObserver(paintingBallList.get(0));
        }
