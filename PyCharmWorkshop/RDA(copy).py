...
while not finish:
    ...
    if msgtype == 1:
        ...
        "Start"
        print
        "Number of channels: " + str(channelCount)
        print
        "Sampling interval: " + str(samplingInterval)
        print
        "Resolutions: " + str(resolutions)
        print
        "Channel Names: " + str(channelNames)
        ...
        if lastBlock != -1 and block > lastBlock + 1:
            print
            "*** Overflow with " + str(block - lastBlock) + " datablocks ***"
        lastBlock = block
...
# Put data at the end of actual buffer
        ...
        if len(data1s) > channelCount * 1000000 / samplingInterval:  # 时间单位是微秒
            index = int(len(data1s) - channelCount * 1000000 / samplingInterval)
            data1s = data1s[index:]

            # 以下计算平均功率(using original data)
            avg = 0
            # Do not forget to respect the resolution !!!  resolution中每一个元素是指通道对电压分辨率，单位是微伏
            for i in range(len(data1s)):
                avg = avg + data1s[i] * data1s[i] * resolutions[i % channelCount] * resolutions[i % channelCount]

            avg = avg / len(data1s)
            print
            "Average power: " + str(avg)
