...
def AStarSearch(Graph, start, end, distances):
    ...
    """
    explored = []
    queue = [(start, distances[start], 0)]
    # TODO: write your code :)
    # Initialize queue here
    return_val = []
    for ele in queue:
        return_val.append(ele[0])
    yield return_val  # yield queue whenever before an element popped out from the queue
    # TODO: write your code :)
    # write your algorithm
    while len(queue) > 0:
        target = queue.pop(0)
        if target[0] in explored:
            continue
        explored.append(target[0])
        if target[0] == end:
            return
        for data in Graph[target[0]]:
            extend = (data[0], data[1] + distances[data[0]] + target[2], data[1] + target[2])
            if extend not in queue:
                queue.append(extend)
