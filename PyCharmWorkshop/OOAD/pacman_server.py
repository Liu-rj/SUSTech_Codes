...
def adj_score(info, id1, id2):
    seg = info.split(' ')
    with shared_lock:
        if seg[0] == '6':
            if int(seg[1]) > int(seg[2]):
                USERS[id1]['score'] += 10
            elif int(seg[1]) < int(seg[2]):
                USERS[id2]['score'] += 10
        elif seg[0] == '7':
            if seg[1] == '4':
                USERS[id2]['score'] += 10
        ...
        while True:
            ...
            if now_player == 0:
                ...
                try:
                    ...
                    self.action_q.append((str(9) + ' ' + str(4) + ' ' + str(e)).encode())
                    return
            ...
            if now_player == 1:
                ...
                try:
                    ...
                    self.action_q.append((str(9) + ' ' + str(5) + ' ' + str(e)).encode())
                    return
    ...
    def run(self):
        while True:
            ...
            if signal == b'' or signal == b'5':
                ...
                return
            # print(signal)
            if signal.decode() in ['0', '1', '2', '3', '4', '6', '7', '8']:
            ...
            # PlayTo an Opponent with username
                ...
                id2 = receive_all(self.conn).decode()
                if not id2:
                    self.conn.send('n'.encode())
                    continue
                current_path = os.getcwd()
                filename1 = os.path.join(current_path, 'AI_' + self.user_name + '.py')
                filename2 = os.path.join(current_path, 'AI_' + id2 + '.py')
                ...
                if flag1 and flag2:
                    ...
                    maze = self.gen_maze()
                    self.send_maze(maze)
                    run_thread = threading.Thread(target=self.main_run, args=(self.user_name, id2, maze))
                    run_thread.start()
                    recv_buf = ''
                    while True:
                        try:
                        ...
                        if self.index < len(self.action_q) and recv_buf == 'y':
                            ...
                            recv_buf = ''
                            info_decode = self.action_q[self.index].decode()
                            if info_decode[0] in ['6', '7', '8', '9']:
                                adj_score(self.action_q[self.index].decode(), self.user_name, id2)
                                self.action_q = []
                                self.index = 0
                                break
                            self.index += 1
                ...
                if user_name in USERS.keys():
                    ...
                    pwd_dic = {'pwd': pwd, 'email': email_address, 'score': 0, 'coin': 0, 'diamond': 0, 'empValue': 0,
                               'level': 0}
                ...
                seg = data.split(' ')
                with shared_lock:
                    user_info = USERS[self.user_name]
                    user_info['score'] = int(seg[0])
                    user_info['coin'] = int(seg[1])
                    user_info['diamond'] = int(seg[2])
                    user_info['empValue'] = int(seg[3])
                    user_info['level'] = int(seg[4])
            elif signal == b'7':
                time.sleep(1)
                user_info = USERS[self.user_name]
                send_data = str(user_info['score']) + ' '
                send_data += str(user_info['coin']) + ' '
                send_data += str(user_info['diamond']) + ' '
                send_data += str(user_info['empValue']) + ' '
                send_data += str(user_info['level'])
                self.conn.send(send_data.encode())
            elif signal == b'8':
                rank = []
                for key in USERS.keys():
                    rank.append((key, USERS[key]['score']))
                rank.sort(key=lambda x: -x[1])
                if len(rank) > 5:
                    rank = rank[0:5]
                send_data = ''
                for ele in rank:
                    send_data += ele[0] + ' ' + str(ele[1]) + ' '
...
if __name__ == "__main__":
    start_listen('10.20.100.145', 5000)
